...
func (b *bucket) checkAndCount(t time.Time) {
	if (t.After(b.from) || t.Equal(b.from)) && t.Before(b.through) {
		b.count = b.count + 1
		log.Println(b)
	}
}
...
func (b *bucket) String() string {
	return fmt.Sprintf("From: %v; To: %v; Counts: %v", b.from, b.through, b.count)
}
...
func (b *secbucket) checkAndCount(t float64) {
	if (t >= float64(b.from)) && t < float64(b.through) {
		b.count = b.count + 1
		log.Println(b)
	}
}

func (b *secbucket) String() string {
	return fmt.Sprintf("From: %v; To: %v; Counts: %v", b.from, b.through, b.count)
}
...
func main() {
	...
	for i := 0; i <= 55; i += 5 {
		buckets = append(buckets, &secbucket{
			from:    i,
			through: i + 5,
			count:   0,
		})
	...
	//  panic(err)
	//}
	fr := time.Date(2020, 7, 31, 0, 00, 00, 00, time.UTC)
	//fr := time.Now().Add(-240 * time.Minute)
	to := time.Date(2020, 7, 31, 23, 59, 00, 00, time.UTC)
	//to := time.Now()
	...
	for _, s := range series {
		...
		//chk, err := chunk.ParseExternalKey(user, chunks[0])
		//if err != nil {
		...
		for _, c := range chunks {
			chk, err := chunk.ParseExternalKey(user, c)
			if err != nil {
				log.Fatal("Failed to parse chunk key:", err)
			}
			if chk.Through < from || chk.Through > through {
				continue
			}
			//	//if chk.Through.After(model.TimeFromUnixNano(time.Date(2020, 7, 31, 07, 10, 00, 00, time.UTC).UnixNano())) &&
			//	//	chk.Through.Before(model.TimeFromUnixNano(time.Date(2020, 7, 31, 07, 15, 00, 00, time.UTC).UnixNano())) {
			//
			//	// Get 2 chunks, the chunk we are iterating on and the next we need timestamps from both
			//	// Check first to see if there is a next element in the list
			//	//if i+1 <= len(stream.chunks)-1 {
			//	//	chk2, err := chunk.ParseExternalKey(user, stream.chunks[i+1])
			//	//	if err != nil {
			//	//		log.Fatal("Failed to parse chunk key:", err)
			//	//	}
			//	//cks, err := cc.GetChunks(ctx, []chunk.Chunk{chk})
			//	//if err != nil {
			//	//	log.Println("Failed to fetch chunk:", err)
			//	//	continue
			//	//}
			...
			//	//ck2 := cks[1]
			//
			//	//it, err := ck1.Data.(*chunkenc.Facade).LokiChunk().Iterator(context.Background(), time.Unix(0, 0), time.Unix(0, math.MaxInt64), logproto.FORWARD, nil)
			//	//if err != nil {
			//	//	panic(err)
			//	//}
			//	//var ck1End time.Time
			//	//for it.Next() {
			//	//	ck1End = it.Entry().Timestamp
			//	//}
			//	//it.Close()
			//	//
			//	//it, err = ck2.Data.(*chunkenc.Facade).LokiChunk().Iterator(context.Background(), time.Unix(0, 0), time.Unix(0, math.MaxInt64), logproto.FORWARD, nil)
			//	//if err != nil {
			//	//	panic(err)
			//	//}
			//	//var ck2Begin time.Time
			//	//it.Next()
			//	//ck2Begin = it.Entry().Timestamp
			//	//it.Close()
			//	//
			//	//cts := (uint64(ck2Begin.UnixNano()) + uint64(chk.Fingerprint)) % uint64(1*time.Hour.Nanoseconds())
			//	//pts := (uint64(ck1End.UnixNano()) + uint64(chk.Fingerprint)) % uint64(1*time.Hour.Nanoseconds())
			//	//enc, err := ck1.Encoded()
			//	//if err != nil {
			...
			//	//}
			//
			//	//log.Println("flush time", chk.Through.Time(), "fp", uint64(chk.Fingerprint), "size", len(enc), "dur", chk.Through.Sub(chk.From).Hours(), "through", chk.Through.Time().UnixNano(), "labels", fc.Metric)
			//	cts := (uint64(chk.Through.Time().UnixNano()) + uint64(chk.Fingerprint)) % uint64(synchronizePeriod.Nanoseconds())
			//	n := chk.Through.Time()
			//	for n.UnixNano() < time.Date(2020, 7, 31, 8, 00, 00, 00, time.UTC).UnixNano() {
			//		n = n.Add(1 * time.Second)
			//		nts := (uint64(n.UnixNano()) + uint64(chk.Fingerprint)) % uint64(synchronizePeriod.Nanoseconds())
			//		if nts < cts {
			//			break
			//		}
			//	}
			syncTime := (float64(uint64(synchronizePeriod.Nanoseconds())-(uint64(chk.Fingerprint)%uint64(synchronizePeriod.Nanoseconds()))) / 1e9) / 60
			for i := range buckets {
				buckets[i].checkAndCount(syncTime)
			}

			// Only count each stream once even if it has multiple chunks in the window
			break

			//if n.After(time.Date(2020, 7, 31, 07, 10, 00, 00, time.UTC)) &&
			//	n.Before(time.Date(2020, 7, 31, 07, 15, 00, 00, time.UTC)) {
			//	log.Println("fp", uint64(chk.Fingerprint), "expectedNextSync", n)
			//}
			...
			//}
			//
			//	fmt.Printf("%v,%v,%v\n", chk.Fingerprint, n.Format("01/02/2006 15:04:05"), chk.Through.Sub(chk.From).Seconds())

			//log.Println("flush time", chk.Through.Time(), "fp", uint64(chk.Fingerprint), "syncIn", n.Sub(chk.Through.Time()))

			//} else {
			//	log.Println("Chunk was in correct range but stream didn't have another chunk")
			//
			//}
	...
	log.Println("From", from.Time(), "Through", through.Time())
	for _, b := range buckets {
		log.Println(b)
	}
...
func main1() {
	...
	bucketStart := time.Date(2020, 7, 31, 06, 00, 00, 00, time.UTC)

	buckets := []*bucket{}

	for bucketStart.Before(time.Date(2020, 7, 31, 8, 00, 00, 00, time.UTC)) {
		buckets = append(buckets, &bucket{
			from:    bucketStart,
			through: bucketStart.Add(5 * time.Minute),
			count:   0,
		})
	...
	//  panic(err)
	//}
	fr := time.Date(2020, 7, 31, 05, 00, 00, 00, time.UTC)
	//fr := time.Now().Add(-240 * time.Minute)
	...
	for _, s := range series {
		...
		for _, c := range chunks {
			...
			if chk.Through < from || through < chk.From {
				continue
			}
			//if chk.Through.After(model.TimeFromUnixNano(time.Date(2020, 7, 31, 07, 10, 00, 00, time.UTC).UnixNano())) &&
			//	chk.Through.Before(model.TimeFromUnixNano(time.Date(2020, 7, 31, 07, 15, 00, 00, time.UTC).UnixNano())) {
...
func analyzeStreams(user string, ctx context.Context, streams []stream, cc chunk.Client) {
	...
	for _, stream := range streams {
		...
		for _, c := range stream.chunks {
			...
			if chk.Through.After(model.TimeFromUnixNano(time.Date(2020, 7, 31, 07, 10, 00, 00, time.UTC).UnixNano())) &&
				chk.Through.Before(model.TimeFromUnixNano(time.Date(2020, 7, 31, 07, 15, 00, 00, time.UTC).UnixNano())) {
				log.Println("flush time", chk.Through.Time(), "fp", uint64(chk.Fingerprint), "labels", fc.Metric)
			}
		}
